...
3. Review blockchain status and insight cards.
4. Upload a sample file.
5. Show the generated proof row and file hash.
6. Verify the hash using the public verification panel.
7. Download the proof certificate.
8. Open Grafana to show operational dashboards.

## GitHub Notes

Recommended files to commit:

- Source code in `frontend/`, `backend/`, `blockchain/`, and `docs/`
...
- `.env`
- local logs
- local blockchain cache/build artifacts

These are already covered by the root `.gitignore`.

## Current MVP Status

Implemented:

- Next.js client-facing dashboard
- Spring Boot proof API
- Spring Authorization Server
- OAuth2/OIDC login flow
- MySQL persistence
- Mock and Ethereum-compatible blockchain adapter paths
- Foundry smart contract package
- Docker Compose local stack
- OTEL, Prometheus, and Grafana dashboards
- Postman collections

Recommended next steps before production:

- Complete an independent smart contract audit
- Configure production OAuth signing keys
- Use managed MySQL
- Deploy contract to Polygon or another Ethereum L2
- Replace local Anvil with production RPC provider
- Add Stripe subscription billing
- Add PDF certificate generation
- Add CI pipeline for frontend, backend, and smart contract tests

## License

This repository is currently prepared as a client-facing MVP/demo project. Add the final license before public release.
